...
{
template<typename T>
inline constexpr typename T::char_type* internal_mreceive(T& ib,typename T::char_type *begin,typename T::char_type *end)
{
	...
	if(static_cast<std::size_t>(ib.end-ib.curr)<n)			//cache miss
	{
		...
		ib.end=receive(ib.native_handle(),ib.beg,ib.beg+ib.size);
		ib.curr=ib.beg;
		n=end-begin;
...
{
	send(out,ib.curr,ib.end);
	ib.curr=ib.end;
}

template<input_stream Ihandler,std::size_t N,std::contiguous_iterator Iter>
inline constexpr Iter receive(istack<Ihandler,N>& ib,Iter begin,Iter end)
{
...
template<input_stream Ihandler,std::size_t N,std::contiguous_iterator Iter>
requires output_stream<Ihandler>
inline constexpr void send(istack<Ihandler,N>& ib,Iter cbegin,Iter cend)
{
	send(ib.native_handle(),cbegin,cend);
}
